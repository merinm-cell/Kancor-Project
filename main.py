...
SMTP_PASSWORD = "ewnb rcya qqxo qyow"  # Ensure this is a valid Gmail App Password
...
password = "#Kancor213"
client_id = f"FastAPI-Backend-{int(time.time())}"  # Unique client ID
...
def on_disconnect(client, userdata, disconnect_flags, reason_code, properties=None):
    ...
    if reason_code != 0:  # Avoid reconnecting if intentional
        while not client.is_connected():
            try:
                ...
                time.sleep(5)  # Reduced from 100s to 5s for faster retries
...
mqtt_client.tls_insecure_set(False)  # Enforce certificate validation
...
# Start MQTT client
try:
    mqtt_client.connect(broker_address, port, keepalive=120)  # Increased keepalive
